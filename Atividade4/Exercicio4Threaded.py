...
def gerar_todos_fractais():
    ...
    thread = threading.Thread(target=samambaia_barnsley)
    threads.append(thread)
    thread.start()

    print("Gerando Conjunto de Mandelbrot...")
    thread = threading.Thread(target=mandelbrot)
    threads.append(thread)
    thread.start()

    print("Gerando Conjunto de Julia...")
    thread = threading.Thread(target=julia)
    threads.append(thread)
    thread.start()

    print("Gerando Curva de Koch...")
    thread = threading.Thread(target=koch_curve)
    threads.append(thread)
    thread.start()

    print("Gerando Árvore Fractal...")
    thread = threading.Thread(target=fractal_tree)
    threads.append(thread)
    thread.start()

    print("Gerando Tapete de Sierpinski...")
    thread = threading.Thread(target=sierpinski_carpet)
    threads.append(thread)
    thread.start()

    print("Gerando Esponja de Menger...")
    thread = threading.Thread(target=menger_sponge)
    threads.append(thread)
    thread.start()
